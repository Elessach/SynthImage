...
void createSecondArmIDList(GLuint id){
	glNewList(id, GL_COMPILE);
		/* rounded square central */
		drawRoundedSquare();
		/* rounded square d'extrémité */
		glPushMatrix();
			glTranslatef(0,-5.,0);
			drawRoundedSquare();
		glPopMatrix();
...
int main(int argc, char** argv) {
  /* Initialisation de la SDL */
	...
	/* Titre de la fenêtre */
	SDL_WM_SetCaption(" L I M A C E ", NULL);
	resize();
	...
	GLuint firstArmID = glGenLists(1);
	createFirstArmIDList(firstArmID);
	GLuint secondArmID = glGenLists(1);
	createSecondArmIDList(secondArmID);
	GLuint thirdArmID = glGenLists(1);
	createThirdArmIDList(thirdArmID);
